...
(deftest
 zdt->date
 (let [now-now (t/zdt-now)] (is (t/date? (t/zdt->date now-now))) nil))
...
(deftest
 date->month
 (let [now-now (t/date-now)] (is (int? (t/date->month now-now))) nil))
...
(deftest
 zdt->month
 (let [now-now (t/zdt-now)] (is (int? (t/zdt->month now-now))) nil))
...
(deftest
 zdt->time
 (let [now-now (t/zdt-now)] (is (t/time? (t/zdt->time now-now))) nil))
